...
public static class FailureSimulationScenario
{
    public static async Task RunAsync(
        ...
        string groupName)
    {
        // 장애 재현에 사용할 Redis Stream key는 Program.cs에서 넘겨받는다.
...
`database`, `streamKey`, `groupName`은 `Program.cs`에서 만들어서 `FailureSimulationScenario.RunAsync(database, streamKey, groupName)`에 넘긴다.
